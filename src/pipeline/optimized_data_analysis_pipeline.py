...
def run_optimized_data_analysis() -> bool:
    ...
    try:
        ...
        with profiler.profile("최적화된_데이터_로드"):
            ...
            data_cache_key = generate_cache_key("historical_data", "load_draw_history")
            ...
            try:
                ...
                data_info = (
                    f"size_{len(historical_data)}_hash_{hash(str(historical_data[:5]))}"
                )
        ...
        with profiler.profile("최적화된_분석기_초기화"):
            try:
                ...
                with ThreadPoolExecutor(max_workers=3) as executor:
                    futures = {
                        ...
                        executor.submit(init_analyzer, "vectorizer"): "vectorizer",
                    ...
                    for future in as_completed(futures):
                        ...
                        logger.info(f"✅ {analyzer_type} 분석기 초기화 완료")
            ...
            try:
                result_dir = Path(
                    config.get("paths", {}).get("result_dir", "data/result")
                )
                cache_dir = Path(config.get("paths", {}).get("cache_dir", "data/cache"))
            ...
            def run_additional_analysis():
                ...
                with ThreadPoolExecutor(
                    ...
                    futures = {
                        ...
                        executor.submit(
                            calculate_number_gaps, historical_data, logger=logger
                        ): "number_gaps",
                        executor.submit(
                            calculate_cluster_distribution,
                            historical_data,
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            logger.error(f"❌ {analysis_type} 분석 실패: {str(e)}")
        ...
        with profiler.profile("결과_저장_검증"):
            ...
            try:
                ...
                with open(analysis_file, "w", encoding="utf-8") as f:
                    json.dump(
                        combined_analysis, f, indent=2, ensure_ascii=False, default=str
                    )
        ...
        logger.info(f"📈 캐시 적중률: {performance_summary['cache_hit_rate']*100:.1f}%")
